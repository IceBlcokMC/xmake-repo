...
option("extern_uv", {showmenu = true, default = false})
...
if has_config("extern_uv") then
    add_requires("nodejs 22.12.0") -- use nodejs butin libuv
    add_packages("nodejs")
    add_defines("LIBUS_USE_LIBUV")
elseif is_plat("windows") or has_config("uv") then
    add_requires("libuv")
    add_packages("libuv")
    add_defines("LIBUS_USE_LIBUV")
end
